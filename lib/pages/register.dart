// ignore_for_file: body_might_complete_normally_catch_error, prefer_const_constructors, library_private_types_in_public_api, avoid_print, use_build_context_synchronously
...
class _RegisterPageState extends State<RegisterPage> {
  ...
  Widget build(BuildContext context) {
    ...
    RegExp regex = RegExp(
        r'^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[!@#$%^&*()_+{}\[\]:;<>,.?/~]).{8,}$');
    if (!regex.hasMatch(value)) {
      return "Enter Valid Password(Min. 8 characters, 1 special, 1 Capital, 1 Numeric)";
    }
    return null;
  },
        onSaved: (value) {
